...
tests_decode theCodes =
  map (uncurry $ Test "decode theCodes" $ decode theCodes)
    ...
    , ( (104,[40,73,80,12,0,72,73,80,12,0,72,79,79,82,65,89,1],51,106)
      , Right "Hip, hip, hooray!"
      )      
    ]
...
--  , ("images/tst.png", ",/:; ~}|{`_^]\\[?>=<!#$%'()*+-. , ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz")
--  , ("images/tst2.png",",/:; ~}|{`_^]\\[?>=<!#$%'()*+-. , ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz a a ,/:; ~}|{`_^]\\[?>=<!#$%'()*+-. , ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz ")
  -- note generate tst.png on my own computer
  ]
